...
int socketError(bool isError, char * msg,
   bool deinitSock = FALSE) {
   int ret = -1;
   if (isError) {
...
// з командного рядка
char * getParameter(char ** argv, int argc,
   const char * paramName,
      char * paramValue, char delim = ' ') {
...
//шлях до файлу
char * getServerPortPath(
      const char * server_port_path,
         char * server, int & port, char * path,
         char portDelim = ': ', char pathDelim = ' / ') {
    char* pportDelim = strchr(server_port_path, ' : '); char* ppathDelim = strchr(server_port_path, ' / ');
         char * pservEnd = (pportDelim) ? pportDelim :
         (ppathDelim) ? ppathDelim : NULL;
         if (pservEnd) {
      ...
      char * getServerPort(char * server_port, char * server,
         int & port, char delim = ': ') {
      ...
      char * getServerPort(char ** argv, int argc,
         const char * paramName,
            char * server, int & port, char delim = ': ') {
      ...
      //з командного рядку
      char * getServerPortPath(char ** argv, int argc,
         const char * paramName, char * server, int & port,
            char * path, char portDelim = ': ', char pathDelim = ' / ') {
         if (!argv || argc <= 1 || !paramName || !server)
            return NULL;
         for (int i = 1; i < argc; ++i) {
            if (stricmp(argv[i], paramName) || i == argc - 1)
               continue;
            return getServerPortPath(argv[i + 1], server, port,
               path, portDelim, pathDelim);
         }
         return server;
      }
      //Функція повертає повний шлях файлу по його імені
      char * extractFilePath(char * filePath,
         const char * fileName,
